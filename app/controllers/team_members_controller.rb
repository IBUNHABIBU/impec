class TeamMembersController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @team_member.update(team_member_params)
        format.html { redirect_to @team_member, notice: "Team member was successfully updated.", status: :see_other }
  ...
  private
    ...
    def set_team_member
      @team_member = TeamMember.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def team_member_params
      params.expect(team_member: [ :name, :title, :bio, :display_order, :photo ])
    end
end
